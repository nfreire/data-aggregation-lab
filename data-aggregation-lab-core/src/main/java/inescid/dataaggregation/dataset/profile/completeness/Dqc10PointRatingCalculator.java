...
public class Dqc10PointRatingCalculator {
	...
	public static double calculate(Model rdfModelOfRec) {
		Set<Resource> rootTypes=Dqc10PointRatingSpec.spec.resources.keySet();
		...
		for(Resource resType: rootTypes) {
			...
			ResIterator roots = rdfModelOfRec.listSubjectsWithProperty(RdfReg.RDF_TYPE, resType);
			while(roots.hasNext()) {
		...
		return (double)found/(double)Dqc10PointRatingSpec.TOTAL_CRITERIA;
	}
}
